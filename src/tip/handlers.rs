...
use crate::issue::db::DBIssue;

pub async fn get_tip_handler(
    id: TipId,
    db_access: impl DBTip + DBIssue,
...
pub async fn get_tip_by_issue_handler(
    id: IssueId,
...
pub async fn get_tips_by_repository_handler(
    id: RepositoryId,
    db_access: impl DBTip,
) -> Result<impl Reply, Rejection> {
    let tips = db_access.get_tips_by_repository(id).await?;
    Ok(warp::reply::json(
        &tips.into_iter().map(TipResponse::of).collect::<Vec<_>>(),
    ))
}
...
pub async fn create_tip_handler(
    ...
    match db_access.get_tip_by_issue(body.issue_id).await? {
        ...
        //TODO: check curator, insert contributor
        None => {
            let id: i32 = body.issue_id.into();
            match db_access.get_issue(id).await? {
                Some(_) => Ok(json(&TipResponse::of(db_access.create_tip(body).await?))),
                None => Err(warp::reject::custom(TipError::IssueNotFound(body.issue_id))),
            }
        }
    }
}

pub async fn update_tip_handler(
    id: TipId,
...
pub async fn delete_tip_handler(
    id: TipId,
    db_access: impl DBTip + DBIssue,
) -> Result<impl Reply, Rejection> {
